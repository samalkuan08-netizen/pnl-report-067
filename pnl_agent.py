...
def parse_1c(path):
    """ОСВ/Анализ счёта. col5=Обороты Дебет, col6=Обороты Кредит.
    Для 7xxx: дебет=расход, кредит=возврат (если Дт!=Кт) -> Прочие доходы.
    Для 6xxx: кредит=доход, дебет=сторно (если Дт!=Кт) -> Прочие расходы."""
    ...
    def emit(acc,it,d,deb,cre,out):
        deb=float(deb) if deb else 0.0; cre=float(cre) if cre else 0.0
        if acc[:1]=='7':
            if deb: out.append((acc,it,'expense',d,deb))
            if cre and abs(cre-deb)>0.005: out.append((acc,it,'return',d,cre))   # возврат
        else:
            if cre: out.append((acc,it,'income',d,cre))
            if deb and abs(cre-deb)>0.005: out.append((acc,it,'income_return',d,deb))  # сторно
    leaves=[]; acct=None; pl=False; item=None; item_e=None; item_f=None; item_got=False; stack=[]
    def flush():
        if pl and item is not None and not item_got and (item_e or item_f):
            emit(acct,item,None,item_e,item_f,leaves)
    for r in range(1,ws.max_row+1):
        ...
        if o<=1:
            ...
            item=None; item_e=None; item_f=None; item_got=False; stack=[]; continue
        if not pl: continue
        if a=='<...>':
        ...
        if div and item is not None:
            ...
            anc=set(dv for (_,dv) in stack)
            if div not in anc and (e or f):
                before=len(leaves); emit(acct,item,div,e,f,leaves)
                if len(leaves)>before: item_got=True
            stack.append((o,div)); continue
        if o==2:
            flush(); item=a; stack=[]; item_e=e; item_f=f; item_got=False; continue
...
def fill_report(template_path,leaves,out_path,month_col='C'):
    ...
    BELOWDH={'TCO':_last_row(tco,'Прочие доходы',7,45),'KPO':_last_row(kpo,'Прочие доходы',7,45),'CONCRETE':_last_row(con,'Прочие доходы',9,55)}
    ...
    for (account,item,kind,div,amount) in leaves:
        ...
        # Возврат по расходному счёту (кредит 7xxx) -> «Прочие доходы» по подразделению
        if kind=='return':
            dk=DIV_KEY.get(div,'CONCRETE'); row=BELOWDH[dk]
            if row: add(SH[dk],row,amount)
            log.append((account,item,div or '—','возврат→Пр.дох',SH[dk].title,'Прочие доходы',amount)); continue
        # Сторно дохода (дебет 6xxx) -> «Прочие расходы»
        if kind=='income_return':
            dk=DIV_KEY.get(div,'CONCRETE'); add(SH[dk],BELOWPR[dk],amount)
            log.append((account,item,div or '—','сторно→Пр.расх',SH[dk].title,'Прочие расходы',amount)); continue
...
def wire_all_sheet(alls,tco,kpo,con):
    ...
    coi=find(con,'Прочие доходы',40,55)  # below-op Прочие доходы на concrete (возвраты)
    if r5:
        for col in 'CDEFGHIJKLMN':
            base=f"='P&L 2026 concrete'!{col}5+'P&L 2026 TCO'!{col}{toi}+'P&L KPO'!{col}{koi}"
            if coi: base+=f"+'P&L 2026 concrete'!{col}{coi}"
            alls[f'{col}{r5}']=base
